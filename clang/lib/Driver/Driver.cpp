...
void Driver::CreateOffloadingDeviceToolChains(Compilation &C,
  ...
  for (StringRef Target : Triples) {
    ...
    for (Action::OffloadKind Kind : Kinds) {
      ...
      auto A = UseLLVMOffload
                   ? (TT.isNVPTX() ? Action::OFK_Cuda : Action::OFK_HIP)
                   : Kind;
...
Action *Driver::BuildOffloadingActions(Compilation &C,
  ...
  bool UseLLVMOffload = Args.hasArg(options::OPT_foffload_via_llvm,
                                    options::OPT_fno_offload_via_llvm, false);
  ...
  for (Action::OffloadKind Kind : OffloadKinds) {
    ...
    if (!UseLLVMOffload &&
        ((Kind == Action::OFK_Cuda && !types::isCuda(InputType)) ||
